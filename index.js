...
function init() {
  function inject(bot) {
    ...
    CubeIterator.prototype.next = function () {
      ...
      var offset = this.point
        .minus(this.max.scaled(0.5).floored())
        .plus(unit[this.side].scaled(this.apothem));
    ...
    function predicateFromMatching(matching) {
      if (typeof matching === "number") {
        ...
        throw new Error(
          "Block Finder: Unknown value for matching: " + matching
        );
      }
    }
    ...
    function findBlockSync(options) {
      ...
      while (
        result.length < options.count &&
        it.apothem <= options.maxDistance
      ) {
    ...
    function findBlock(options) {
      return new Promise((resolve, reject) => {
        options = optionsWithDefaults(options);
        const it = new OctahedronIterator(options.point);
        const result = [];
        setImmediate(() => {
          while (
            result.length < options.count &&
